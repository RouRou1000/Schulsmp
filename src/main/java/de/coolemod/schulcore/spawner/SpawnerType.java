...
/**
 * Definiert alle Spawner-Typen und ihre Drop-Tables
 * Spawner erzeugen KEINE echten Mobs, nur Items (Performance-optimiert)
 */
public enum SpawnerType {
    ...
    SKELETON(EntityType.SKELETON, "§7Skeleton Spawner", Material.BONE,
        new Drop(Material.BONE, 1, 3, 100),
        new Drop(Material.ARROW, 1, 3, 80),
        new Drop(Material.BOW, 1, 1, 5)),
    
    CREEPER(EntityType.CREEPER, "§aCreeper Spawner", Material.GUNPOWDER,
        new Drop(Material.GUNPOWDER, 1, 2, 100),
        new Drop(Material.MUSIC_DISC_CAT, 1, 1, 1)),
    
    SPIDER(EntityType.SPIDER, "§8Spider Spawner", Material.STRING,
        new Drop(Material.STRING, 1, 2, 100),
        new Drop(Material.SPIDER_EYE, 1, 1, 50)),
    
    CAVE_SPIDER(EntityType.CAVE_SPIDER, "§2Cave Spider Spawner", Material.SPIDER_EYE,
        new Drop(Material.STRING, 1, 2, 100),
        new Drop(Material.SPIDER_EYE, 1, 2, 75)),
    ...
    COW(EntityType.COW, "§fCow Spawner", Material.LEATHER,
        ...
        new Drop(Material.LEATHER, 1, 2, 100)),
    ...
    CHICKEN(EntityType.CHICKEN, "§eChicken Spawner", Material.FEATHER,
        ...
        new Drop(Material.FEATHER, 1, 2, 100)),
    
    IRON_GOLEM(EntityType.IRON_GOLEM, "§7§lIron Golem Spawner", Material.IRON_INGOT,
        new Drop(Material.IRON_INGOT, 3, 5, 100),
        new Drop(Material.POPPY, 1, 2, 50)),
    
    WITCH(EntityType.WITCH, "§5Witch Spawner", Material.GLASS_BOTTLE,
        new Drop(Material.GLASS_BOTTLE, 1, 2, 40),
        new Drop(Material.GLOWSTONE_DUST, 1, 2, 30),
        new Drop(Material.REDSTONE, 1, 2, 30),
        new Drop(Material.SPIDER_EYE, 1, 1, 20),
        new Drop(Material.SUGAR, 1, 2, 20),
        new Drop(Material.GUNPOWDER, 1, 2, 25),
    ...
    DROWNED(EntityType.DROWNED, "§3Drowned Spawner", Material.ROTTEN_FLESH,
        ...
        new Drop(Material.GOLD_INGOT, 1, 1, 10),
        new Drop(Material.COPPER_INGOT, 1, 2, 15),
        new Drop(Material.TRIDENT, 1, 1, 2)),
    
    GUARDIAN(EntityType.GUARDIAN, "§bGuardian Spawner", Material.PRISMARINE_SHARD,
        new Drop(Material.PRISMARINE_SHARD, 1, 2, 100),
        new Drop(Material.PRISMARINE_CRYSTALS, 1, 1, 50),
    ...
    /**
     * Generiert Drops basierend auf Stack-Größe
     * @param stackSize Anzahl der gestackten Spawner
     * @return Liste der generierten Items
     */
    public List<ItemStack> generateDrops(int stackSize) {
        Map<Material, Integer> dropMap = new HashMap<>();
        
        // Pro Spawner im Stack werden Drops generiert
        ...
        for (Map.Entry<Material, Integer> entry : dropMap.entrySet()) {
            ...
            // Teile in Stacks von max 64 auf
            while (total > 0) {
    ...
    public static SpawnerType fromEntityType(EntityType type) {
        for (SpawnerType st : values()) {
            if (st.entityType == type) return st;
        }
        return null;
    }
