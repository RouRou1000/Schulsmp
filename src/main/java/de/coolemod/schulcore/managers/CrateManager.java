...
public class CrateManager {
    ...
    public CrateManager(SchulCorePlugin plugin) {
        this.plugin = plugin;
        loadCrates();
    }
    ...
    public void openCrateAnimated(Player p, String crateId, boolean skipKeyCheck) {
        ...
        BukkitTask task = Bukkit.getScheduler().runTaskTimer(plugin, () -> {
            ...
            if (count >= c.cycles) {
                ...
                // Deterministic armor set based on tier (like a shop)
                List<ItemStack> rewards = createTierArmorSet(c.tier);
    ...
    /**
     * Erstellt ein einzelnes Rüstungsteil mit Tier-Enchantments.
     * @param tier "basic", "rare", oder "legendary"
     * @param piece "HELMET", "CHESTPLATE", "LEGGINGS", oder "BOOTS"
     */
    public ItemStack createSingleArmorPiece(String tier, String piece) {
        ...
        Material mat;
        try { mat = Material.valueOf(prefix + piece); } 
        catch (IllegalArgumentException ex) { mat = Material.DIAMOND_CHESTPLATE; }
        
        ItemStack item = new ItemStack(mat);
        boolean isHighTier = tier.equalsIgnoreCase("legendary") || tier.equalsIgnoreCase("rare");
        
        int protLevel = isHighTier ? 4 : 2;
        int unbreakingLevel = isHighTier ? 3 : 1;
        
        org.bukkit.enchantments.Enchantment prot = getEnchantSafe("PROTECTION_ENVIRONMENTAL", "PROTECTION");
        org.bukkit.enchantments.Enchantment unbreaking = getEnchantSafe("DURABILITY", "UNBREAKING");
        org.bukkit.enchantments.Enchantment mending = getEnchantSafe("MENDING", "MENDING");
        
        if (prot != null) item.addUnsafeEnchantment(prot, protLevel);
        if (unbreaking != null) item.addUnsafeEnchantment(unbreaking, unbreakingLevel);
        if (isHighTier && mending != null) item.addUnsafeEnchantment(mending, 1);
        
        // Helmet extras
        if (piece.equals("HELMET") && isHighTier) {
            org.bukkit.enchantments.Enchantment aqua = getEnchantSafe("WATER_WORKER", "AQUA_AFFINITY");
            org.bukkit.enchantments.Enchantment resp = getEnchantSafe("OXYGEN", "RESPIRATION");
            if (aqua != null) item.addUnsafeEnchantment(aqua, 1);
            if (resp != null) item.addUnsafeEnchantment(resp, 3);
        }
        
        // Boots extras
        if (piece.equals("BOOTS") && isHighTier) {
            org.bukkit.enchantments.Enchantment feather = getEnchantSafe("PROTECTION_FALL", "FEATHER_FALLING");
            org.bukkit.enchantments.Enchantment depth = getEnchantSafe("DEPTH_STRIDER", "DEPTH_STRIDER");
            if (feather != null) item.addUnsafeEnchantment(feather, 4);
            if (depth != null) item.addUnsafeEnchantment(depth, 3);
        }
    ...
    /**
     * Erstellt ein vollständiges Rüstungsset basierend auf dem Tier.
     * Legendary + Rare: Prot IV, Unbreaking III, Mending + Helmet/Boots Extras
     * Basic: Prot II, Unbreaking I (weniger Enchants)
     */
    private List<ItemStack> createTierArmorSet(String tier) {
        ...
        // Helper: get enchantment safely
        org.bukkit.enchantments.Enchantment prot = getEnchantSafe("PROTECTION_ENVIRONMENTAL", "PROTECTION");
        org.bukkit.enchantments.Enchantment unbreaking = getEnchantSafe("DURABILITY", "UNBREAKING");
        org.bukkit.enchantments.Enchantment mending = getEnchantSafe("MENDING", "MENDING");
        org.bukkit.enchantments.Enchantment aquaAffinity = getEnchantSafe("WATER_WORKER", "AQUA_AFFINITY");
        org.bukkit.enchantments.Enchantment respiration = getEnchantSafe("OXYGEN", "RESPIRATION");
        org.bukkit.enchantments.Enchantment featherFalling = getEnchantSafe("PROTECTION_FALL", "FEATHER_FALLING");
        org.bukkit.enchantments.Enchantment depthStrider = getEnchantSafe("DEPTH_STRIDER", "DEPTH_STRIDER");

        // Apply base enchants to all pieces
        for (ItemStack piece : new ItemStack[]{helmet, chest, legs, boots}) {
            if (prot != null) piece.addUnsafeEnchantment(prot, protLevel);
            if (unbreaking != null) piece.addUnsafeEnchantment(unbreaking, unbreakingLevel);
            if (isHighTier && mending != null) {
                piece.addUnsafeEnchantment(mending, 1);
            }
        }

        // Helmet extras
        if (isHighTier) {
            if (aquaAffinity != null) helmet.addUnsafeEnchantment(aquaAffinity, 1);
            if (respiration != null) helmet.addUnsafeEnchantment(respiration, 3);
        }

        // Boots extras
        if (isHighTier) {
            if (featherFalling != null) boots.addUnsafeEnchantment(featherFalling, 4);
            if (depthStrider != null) boots.addUnsafeEnchantment(depthStrider, 3);
        }
    ...
    /**
     * Enchantment safe lookup: tries legacy name first, then modern name.
     */
    private org.bukkit.enchantments.Enchantment getEnchantSafe(String legacyName, String modernName) {
        org.bukkit.enchantments.Enchantment e = org.bukkit.enchantments.Enchantment.getByName(legacyName);
        if (e == null) e = org.bukkit.enchantments.Enchantment.getByName(modernName);
        return e;
    }
