...
public class SidebarManager {
    ...
    public SidebarManager(SchulCorePlugin plugin) {
        this.plugin = plugin;
    }
    ...
    private void updateFor(Player p) {
        ...
        String money = "§6⛃ §7ɢᴇʟᴅ§8: §a" + NumberFormatter.formatMoney(plugin.getEconomy().getBalance(p.getUniqueId()));
        ...
        String kills = "§c⚔ §7ᴋɪʟʟѕ§8: §e" + NumberFormatter.formatInt(plugin.getStats().getKills(p.getUniqueId()));
        obj.getScore(kills).setScore(5);
        
        String deaths = "§4☠ §7ᴅᴇᴀᴛʜѕ§8: §c" + NumberFormatter.formatInt(plugin.getStats().getDeaths(p.getUniqueId()));
