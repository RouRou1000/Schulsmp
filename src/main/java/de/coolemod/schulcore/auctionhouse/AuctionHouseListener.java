package de.coolemod.schulcore.auctionhouse;
...
public class AuctionHouseListener implements Listener {
    ...
    public AuctionHouseListener(SchulCorePlugin plugin, AuctionHouse ah) {
        this.plugin = plugin;
        this.ah = ah;
    }
    ...
    public void onClick(InventoryClickEvent e) {
        ...
        // Block dangerous click types in browse/my_auctions GUIs (prevent item loss)
    ...
    private void handlePriceSign(SignChangeEvent e, Player player, AuctionHouse.CreateSession session) {
        ...
        try {
            ...
            if (price <= 0) {
                ...
                player.sendMessage("§8┃ §7Beispiele: §f100§7, §f10k§7, §f1.5m");
                Bukkit.getScheduler().runTask(plugin, () -> {
                    player.openInventory(ah.createNewAuctionGUI(player.getUniqueId()));
                });
                return;
