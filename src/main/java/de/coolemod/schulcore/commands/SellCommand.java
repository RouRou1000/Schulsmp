...
public class SellCommand implements CommandExecutor {
    ...
    public SellCommand(SchulCorePlugin plugin) { 
        this.plugin = plugin; 
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            new de.coolemod.schulcore.gui.SellGUI(plugin).open(p);
            return true;
        }
    ...
    private boolean sellHand(Player p) {
        ...
        int shardsEarned = NumberFormatter.calculateShardsFromMoney(total);
        if (shardsEarned > 0) {
        ...
        p.sendMessage("§8  ▸ §7Erhalten: §a+" + NumberFormatter.formatMoney(total));
        if (shardsEarned > 0) {
            p.sendMessage("§8  ▸ §7Shards: §d+" + shardsEarned + " §8(§71 pro $1k§8)");
        }
