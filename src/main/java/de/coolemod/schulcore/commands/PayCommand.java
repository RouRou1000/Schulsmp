...
public class PayCommand implements CommandExecutor {
    ...
    public PayCommand(SchulCorePlugin plugin) { this.plugin = plugin; }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (amount < 0) {
            ...
            p.sendMessage(prefix + "§7Beispiele: §f100§7, §f10k§7, §f1.5m§7, §f1b");
            p.playSound(p.getLocation(), Sound.ENTITY_VILLAGER_NO, 1f, 1f);
            return true;
        }
        ...
        p.sendMessage("  §7Neuer Kontostand: " + NumberFormatter.formatMoney(plugin.getEconomy().getBalance(p.getUniqueId())));
        p.sendMessage("");
        ...
        target.sendMessage("  §7Neuer Kontostand: " + NumberFormatter.formatMoney(plugin.getEconomy().getBalance(target.getUniqueId())));
        target.sendMessage("");
